...
W1：夹具 `app.vue` 用了 MSS 不认的裸类型选择器，包**装不进去**、CI 绿、到现场才会发现全局空跑。
**夹具最危险的失败模式不是写错一行，而是"夹具与产品走两条不同的路"。**

- 凡夹具/示例（`demo-apps/**`、`docs/` 里的示例包、runbook 里贴的片段），必须有一条**机械闸**
  让它走真链路：`PackageReader` → `Manifest` → `SfcCompiler` →（带后端的）`ScriptStaticCheck`；
  标准形态就是 `docs/WindowFixturesTest`（夹具坏 → `assertTestWindowFixturesTest` 红）。
- 改了夹具/示例必须有**变异验收**：把旧的错误写法改回去，断言必须红（不能只验"改后绿"）。
- 不能机械化覆盖的片段（runbook 里贴的代码块）：改动时人工走一遍同一链路，或在旁边注明"未走闸"。

## 六、签名 / 未签名各装一次（S16b #14–#16 顺手收口）

在真客户端上按顺序做，每步留原始输出/截图：

1. **已签名包**：`signApp` 正常签 → 安装 → 记录：
   `INSTALL_NOTE` 是否显示；「已签名」处**不得**有任何暗示内容安全的图标或文案。
2. **改一字节重压**：应判「签名无效」并**拒绝安装**。
3. **只换 `META/sig.json`**：应判「作者密钥变了」，且要**输入确认短语**（不是点一下）。
4. **未签名包**：把源目录直接压一个 zip（不跑 `signApp`，即没有 `META/sig.json`）→ 记录当前闸下的实际行为：
   装得上/装不上、提示文案是什么。
   - 装不上就明确记「**必须签名**」+ 证据（这条决定作者上手成本，别含糊）。
   - 装得上就记「未签名可装」并写出警告文案。
5. 证据里注明每一步用的是哪个 zip（带签名 / 不带签名）。
