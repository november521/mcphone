 ...
 * 光用 javac 编。跑法（要挂 Forge 的编译类路径）：见 docs/PORTING.md 的"复现这套筛法"一节，
 * 把 out 目录和本文件一起编了即可。
 */
public class ChatMessageCodecTest {
    ...
    public static void main(String[] args) {
        ...
        ChatMessage.encode(sticker, animBuf);
        eq(ChatMessage.decode(animBuf), sticker, "动图消息过网络应当原样回来");
        ...
        ChatMessage.encode(photo, buf);
        eq(ChatMessage.decode(buf), photo, "图片消息过网络应当原样回来");
        eq(buf.readableBytes(), 0, "解完应当正好读空，多一个字节都是格式对不上");

        FriendlyByteBuf buf2 = new FriendlyByteBuf(Unpooled.buffer());
        ChatMessage.encode(text, buf2);
        eq(ChatMessage.decode(buf2), text, "文本消息过网络应当原样回来");
