...
| `frontendDigest` | `FrontendDigest.of(本地包)`，算不出时回退握手那格 | 详情页灰字"界面已被本地修改" | **否**（§13.3：只回显，恶意客户端可伪造） |
...
整条 push）。客户端拿不到某条部署时等同"本服没有它"（`backend.available = false`），这是设计好的
降级方向（少信息，不是错信息）。
...
- `/mcphoneclient handshake`：打印 serverId / epoch / 部署表快照。
