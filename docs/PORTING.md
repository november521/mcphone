> **这是一份记录，不是待办。** 它写于两支还分开的时候，扫的是当时
> `MCphone Neoforge 1.21.1/src/main/java` 下 215 个文件；两支合并之后目录结构与
> 文件数都变了，**里面的数字不要照着用**。留着是因为这套多目标结构的判据就是从
> 这份扫描里长出来的：哪些代码与加载器无关、哪些只与版本有关，第一次是在这里量出来的。
>
> 现在的判据与分层见 [`shared/PLATFORM-SEAMS.md`](../shared/PLATFORM-SEAMS.md)。
