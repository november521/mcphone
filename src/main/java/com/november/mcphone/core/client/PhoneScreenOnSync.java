 ...
 * 判断依据就得放在同步得出去的地方——物品自己身上，而那只有服务端写得了。这个类是那条链的
 * 第一环：开机/关机时报一次；服务端写进物品；物品随手持同步给所有看得见这只手的人；
 ...
 *
 * <pre>
 *   开机  new PhoneScreen(...)     —— 全屏开的、HUD 挂上的，都从这儿来
 *   关机  PhoneScreen.shutdown()   —— 被别的界面顶掉、ESC、点机身外、手机离开副手，
 *                                    最后都汇到这一句
 *   挪位  PhoneScreen.relocate(..) —— 手机在身上换了个地方，人还开着它
 * </pre>
 *
 ...
 * 只是白发一个包、白写一次 NBT。所以位置不是"在手上"就当没亮——包也不发。
 */
public final class PhoneScreenOnSync {
     ...
     * 离开世界：把去重用的记忆清空。
     *
     * 不发包——连接正在拆。清掉是为了下一局能重发：不清的话，进新世界开手机时会因为
     * "和上次一样"被判成没变。
     *
     * 【这一支还要多做一件事】：那边写的是只同步不落盘的组件，玩家下次进来手机自然是灭的；
     * 这一支写的是 NBT，会落盘。所以退出时的清理由服务端那一侧的
     * {@code PhoneScreenOnCleanup} 负责，见那个类。
     */
