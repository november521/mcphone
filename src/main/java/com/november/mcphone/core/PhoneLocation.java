...
public sealed interface PhoneLocation {
    ...
    /**
     * 挂在 Curios 的饰品槽里（仅 NeoForge 端会产生）。
     *
     * Fabric 版留着这个类型：TYPE_CURIO 那个字节已经发出去过（跨端联机、
     * 存档里的旧数据都可能出现），线格式不能挪用。但 Fabric 上没有 Curios，
     * 解出来也一定对不上任何东西 —— resolve 返回空堆、writeBack 不动，
     * 上层的 isPhone 校验会把它当"位置已失效"安全丢弃。
     */
    ...
    static Optional<PhoneLocation> find(Player player) {
        ...
        // （原 NeoForge 版还会再扫一遍 Curios 饰品槽；Curios 在 1.21.1 的
        // Fabric 上没有构建，Fabric 版只认手上与背包。）
