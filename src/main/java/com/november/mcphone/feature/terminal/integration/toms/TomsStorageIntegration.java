 ...
 * ——它把绑定的维度与坐标存在物品的 NBT 里（1.21.1 那边是 DataComponent，这边是
 * 1.20.1 的老一套，对我们没有任何差别），打开时等价于"隔空对着那个方块右键"。
 ...
 *
 * <h2>三家里唯一一家两支逐字相同的</h2>
 *
 * {@code com.tom.storagemod.item.WirelessTerminal} 这个接口在 1.20-1.6.1 与 1.20-1.7.1
 * 上都是同样三个方法（{@code getRange} / {@code open} / {@code canOpen}），与 2.4.2 那边
 * 一模一样。基础无线终端的 {@code open} 方法体仍是一句 {@code return}、{@code canOpen}
 * 仍是恒 false —— 两头的字节码都核过。所以这个类相对 main 只改了一处注释里的名词
 * （DataComponent → NBT），代码一个字没动。
 */
