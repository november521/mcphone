...
public class TerminalSlotMenu extends AbstractContainerMenu {
     ...
     * 客户端与服务端共用这一个构造。
     *
     * 不需要额外的开局数据：卡槽的内容由玩家数据自己同步过来（见 TerminalSlot），不必再
     * 随开菜单的包发一遍。菜单开着的时候原版的容器同步包还会再兜一层——它下发的那一份
     * 最终也是写回同一处（{@link TerminalSlotContainer#setItem}），两条路不会各存一份。
     */
    public TerminalSlotMenu(int containerId, Inventory playerInventory) {
        ...
        addSlot(new Slot(terminal, 0, SLOT_X, SLOT_Y) {
             ...
             * 而且这一支上有两种不一样：
             *
             *   Tom's 的基础无线终端  是终端，但没有"隔空打开"这回事（{@code canOpen}
             *                        恒为 false，{@code open} 的方法体是一句 return）
             *   RS 的终端            开得了，但这个版本的 RS 表达不了"东西在手机卡槽里"
             *                        （见 {@code RefinedStorageIntegration}）
             *
             * 装得进去却点不开，比装不进去更难解释，所以两种都在这一步被挡住。
             *
