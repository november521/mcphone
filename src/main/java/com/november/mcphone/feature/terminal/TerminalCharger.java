...
public final class TerminalCharger {
    ...
    /** 由 MCphone 挂到 END_SERVER_TICK 上，每个在线玩家调一次 */
    ...
    public static void topUp(ServerPlayer player) {
        ...
        // Fabric 上没有 NeoForge 的能力系统，能量走 TR Energy（RS 的 Fabric 版就是它；
        // AE2/ae2wtlib 在 1.21.1 上没有 Fabric 构建，它们的终端在 Fabric 版用不到）。
        // withConstant 让收进来的电落在终端这个 ItemStack 本体上
        EnergyStorage energy = EnergyStorage.ITEM.find(terminal,
                ContainerItemContext.withConstant(terminal));
        if (energy == null || !energy.supportsInsertion()) return;

        long missing = energy.getCapacity() - energy.getAmount();
        if (missing <= 0) return;

        // 满了就不必再同步。这一句也顺带兜住"说得收、实际一点都收不进"的实现。
        // TR Energy 的插入要走事务：commit 了才算数，不 commit 等于没插
        try (Transaction tx = Transaction.openOuter()) {
            if (energy.insert(missing, tx) > 0) {
                tx.commit();
