...
/** 记事本的服务端业务逻辑，与网络包分开；数据存在玩家 Capability {@link ModCapabilities}（那一支是 ModAttachments#NOTES） */
public final class NoteService {
    ...
    public static boolean saveNote(ServerPlayer player, int id, String rawBody) {
        ...
        ModCapabilities.of(player).setNotes(updated);
        return true;
    }
    ...
    private static NoteList notes(ServerPlayer player) {
        return ModCapabilities.of(player).notes();
    }
}
