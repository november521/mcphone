...
| 💬 美西螈 | 手机上的聊天 App。双向好友（申请 / 同意 / 拒绝）、会话列表（未读数、在线状态、最后一条预览）、气泡式会话界面。消息存进世界存档，对方离线也能发。在线好友名字后面有个小图标，点一下就传送到他身边 |
...
**传送到好友身边**：在线好友的名字后面有一个小图标，**点它**人就过去了，手机同时自动关掉。落点与原版 `/tp 玩家 玩家` 一模一样——落在他站的那个坐标上，朝向也和他一致。跨维度也传，他在下界你在主世界照样过去。离线的人没有这个图标；鼠标停上去图标会亮起来，第二行那句消息预览临时换成「传送到他身边」，省得对着一个 8 像素的图标猜。
...
| `chat/teleport.png` | 会话列表里在线好友名字后面那个「传送到他身边」的小图标 | 8×8（按实际绘制尺寸画，这里不做平滑缩放） | 画 → 字符 |
