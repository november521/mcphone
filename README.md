...
| 🏪 应用商店 | 安装 / 卸载 App，安装状态持久化；系统 App 标灰不可卸载。首格是「联动App」，见下方 |
...
但那样玩家也就无从知道自己缺了什么。所以应用商店的第一格是「联动App」，点进去一行一个列着：图标、名字、需要哪个模组、已装还是未装。前置没齐的整行压暗，看得见、点不动。手机里「设置 → 关于」也会列出这些模组各自装没装。
