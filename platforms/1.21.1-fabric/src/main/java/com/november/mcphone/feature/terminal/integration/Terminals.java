 ...
 * 谁来调 discover
 *
 * 懒触发：第一次被问到（{@link #active()} / {@link #anyPresent()}）时自己挑，
 * {@link #discover()} 幂等。Fabric 没有 FMLCommonSetup 那个"全体构造完"的节点，
 * 但【客户端】的第一问必然发生在 onInitializeClient 阶段 —— 那时所有模组的
 * 主阶段初始化都跑完了，RS 的 API 代理已经挂上；【服务端】的第一问在进世界后
 * （开菜单/收包），更晚。都比"RS 还没构造完"早不了 —— 这就是想要的效果。
 *
 * 一个都没装怎么办
 *
...
public final class Terminals {
    ...
    /** 已经挑过了。见 {@link #discover()} */
    private static volatile boolean discovered = false;

    /**
     * 挑出装了的那几家，各自做一次性注册。幂等，想调几次调几次 ——
     * 只有第一次真正干活。
     *
     * 为什么不在模组构造里做
     *
     * RS 的 {@code RefinedStorageApi.INSTANCE} 是个<b>代理对象</b>，真正的实现由 RS 自己的
     * 初始化挂上去。在构造里调它，拿到的是还没挂 delegate 的空壳。懒触发（见类注释）
     * 保证第一次真正干活时所有模组都构造完了，不需要靠依赖顺序推理。
     */
    public static void discover() {
        if (discovered) return;
        synchronized (Terminals.class) {
            if (discovered) return;
            discover0();
            discovered = true;
        }
    ...
    /** 真正的挑法，从 discover 的双检锁里调 */
    private static void discover0() {
    ...
    /** 装了的那几家，第一次问到时才挑。顺序就是 {@link #discover0()} 里的顺序，也就是认领终端时的优先级 */
