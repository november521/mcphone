 ...
 *
 * <b>这不只是签名不同，两边落盘的形状也不同</b>：这一支在空的时候整个不写那个字段，另一支总会写出来。
...
public final class StackCodecs {
    ...
    public static MapCodec<ItemStack> optionalStackField(String name) {
        // 1.20.1 只有不接受空栈的 ItemStack.CODEC。空的时候【整个不写这个字段】，
        // 读回来是 Optional.empty —— 语义与那一支的空栈一致，落盘形状则是这一支原有的那种。
        return ItemStack.CODEC.optionalFieldOf(name).xmap(
                opt -> opt.orElse(ItemStack.EMPTY),
                stack -> stack.isEmpty() ? Optional.<ItemStack>empty() : Optional.of(stack));
    }
}
