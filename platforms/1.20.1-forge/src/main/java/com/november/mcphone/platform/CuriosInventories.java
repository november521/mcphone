 ...
 * <h2>为什么值得一层</h2>
 *
 * 两个版本的 Curios 在这一处返回的东西不同：这一支（Curios 5.x）给的是 Forge 的 {@code LazyOptional}，另一支直接给 {@code Optional}。
...
public final class CuriosInventories {
    ...
    public static Optional<ICuriosItemHandler> of(LivingEntity entity) {
        // 5.x 给的是 LazyOptional，resolve() 把它变成普通 Optional。
        // 这里就地 resolve 是对的：调用方拿到之后马上就读，不存着。
