...
public final class CuriosCompat {
    ...
    /**
     * 【1.20.1 与 1.21.1 在这里不一样】：Curios 5.x（1.20.1）的
     * getCuriosInventory 返回的是 Forge 的 LazyOptional，不是 java.util.Optional，
     * 而 LazyOptional 【没有 flatMap】。先 resolve() 换成标准 Optional 再往下接。
     *
     * 9.x（1.21.1）那边已经改成返回 Optional 了，所以那一支不需要这一步。
     */
    ...
    private static ItemStack getEquippedInternal(LivingEntity entity, String slotId, int index) {
        // resolve() 的理由同 findEquippedInternal
        return CuriosApi.getCuriosInventory(entity).resolve()
