 ...
 * }</pre>
 *
 * <p>上面这句话逐字来自 neoforge 那一份，它抄的是神化 Apotheosis 1.21 的
 * {@code AdventureKeys.handleKeys}：那个键位默认绑 Ctrl + T，"世界层级选择"那个界面就是它开的，
 * 也正是遥控器点不动的那一个。本目标<b>没有</b>去核对某个模组在 1.20.1 上的源码，所以这里钉的
 * 是形状而不是某一个模组：{@code isConflictContextAndModifierActive()} 是 Forge 系补丁给
 * {@code KeyMapping} 加的方法（1.20.1 上由 {@code IForgeKeyMapping} 提供），
 * 凡是那么写的模组都落进这一节，修复也不针对某一个模组。
 *
 ...
 *       （神化 1.20.1 的 {@code WORLD_TIERS_ARENT_REAL} 与 1.21 的那个是同一个形状），
 *       点击会被它取走却什么都不做，于是被报成成功 —— 这一点写在 {@code KeyTrigger}
 *       的覆盖边界一节里。</li>
 ...
 *       {@code KeyMapping} 的，AT 能不能稳定打上去没有验过（本机编不了这个目标）；
 ...
 * 后者也只有 neoforge / forge 有，fabric 那三个方法全是空转（见那份的类注释）。
 */
public final class HotkeyBackend {
     ...
     *
     * <p>本目标这里用的是 Forge 的 {@code setKeyModifierAndCode}：它先把这个 mapping 从内部
     * 那张按键索引的冲突表（{@code MAP}）里摘下来，改完再按<b>传进去的那个键</b>登记回去，
     * 而且体内没有 {@code resetMapping()} —— 不会把当前值拉回默认值、让这次让开当场作废。
     * 所以这里传 {@code mapping.getKey()}：登记位置一个字不变，动的只有修饰键那一栏。
     *
     ...
     *
     * <p>{@code setKeyModifierAndCode} 体内另有一句规范化：传进去的修饰键若正好
     * {@code matches} 那个键码，就会被改成 {@code NONE}。这里传的是原先那个修饰键加这个键位
     * <b>自己</b>的键码（CONTROL 与 T），而 CONTROL 不认 T —— 所以收回这一步撞不上那句规范化，
     * 放回去的 Ctrl 不会被它悄悄抹掉。这一条由平台 {@code docs/} 里那份断言测试钉住。
     */
