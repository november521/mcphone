...
public record ImageBody(UUID image, int width, int height, int frames, int frameMs)
    ...
    public static void encode(ImageBody body, FriendlyByteBuf buf) {
        buf.writeUUID(body.image());
        buf.writeVarInt(body.width());
        buf.writeVarInt(body.height());
        buf.writeVarInt(body.frames());
        buf.writeVarInt(body.frameMs());
    }

    public static ImageBody decode(FriendlyByteBuf buf) {
