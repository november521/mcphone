 ...
 *
 * <h2>绝不能用客户端那个"按服务器 IP 归一"的分组键（§13.9 的 grep 判据）</h2>
 *
 * 那个键是 {@code "server_" + sanitize(server.ip)}：会碰撞（{@code 1.2.3.4:25565} 与 {@code 1.2.3.4_25565}
 * 归一后同一个键），而且同一 IP 换服主就复用（旧授权会落到新服主头上）。它作为"安装状态的便捷分组键"
 * 没问题，但授权、保险箱、敏感 KV <b>一律不许用它</b>（§13.5/§13.9）。为了那条 grep 判据保持可执行，
 * 这里不写它的方法名。
 *
