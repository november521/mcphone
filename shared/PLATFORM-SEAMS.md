...
代码按共用范围分四层，各层的准入条件不同：

| 目录 | 供谁使用 | 层中不得出现 | 校验任务 |
|---|---|---|---|
| `shared/` | 全部目标 | 四条判据均不得命中 | `verifySharedIsTargetNeutral` |
| `layers/version/<层名>/` | 挂载该层的目标 | 绑定加载器的构造 | `verifyLayer<层名>` |
| `layers/loader/<层名>/` | 挂载该层的目标 | 绑定 Minecraft 版本的构造 | `verifyLayer<层名>` |
| `platforms/<目标名>/` | 单个目标 | —— | —— |

中间两层的存在理由：仅受版本约束的类型（手写编解码、各类网络包）与加载器无关。
...
- [`versions/layers.json`](../versions/layers.json) —— 有哪些层，各自的目录、种类与状态
...
由 `since` / `loader` 哪个字段在场决定，二选一：

| 种类 | 字段 | 供谁共用 | 层中不得出现 |
|---|---|---|---|
| 版本层 | `since` | 同一批 Minecraft 版本的三个加载器 | 绑定加载器的构造 |
| 加载器层 | `loader` | 同一加载器的各个 Minecraft 版本 | 绑定版本的构造 |

二者对称：各自容纳的正是对方挡下的那一半。两个字段都不写，构建无从判断该层禁哪个轴；
两个都写，则是两件事挤在一层。构建与 CI 均校验这一条。
...
| 层 | 种类 | 边界 | 内容 |
|---|---|---|---|
| `1.20.5+` | 版本层 | 1.20.5 起原版才有 `net.minecraft.network.codec`、`CustomPacketPayload`、数据组件 | 54 个 java |
| `1.21+` | 版本层 | 数据包格式 48 将目录名由复数改为单数（`recipes`→`recipe` 等） | 5 份资源 |
...
三个加载器层为空（`populated: false`），机制先行建立，内容待定。本仓目前仅一个目标可构建，
没有第二个同加载器目标可供逐文件比对，无从判断某段 NeoForge 代码在 20.1 与 21.1 上
是否一致 —— 加载器 API 在大版本之间的改动多于原版。待第二个同加载器目标并入后，
按版本层同一办法处理：逐文件比对，**逐字相同者方可迁入**。
